...
def _get_column_types(data):
    ...
    categorical_columns = [
        col for col in approp_columns if 2 < data[col].nunique() < 10
    ]
    ...
    numerical_columns = [
        ...
        if col in approp_columns and data[col].dtype == float
    ]
...
def get_column_(
    data, list_of_column_names, name, raise_error=True, modify_inplace=False
):
    column = [x for x in data.columns if x.lower() in list_of_column_names]
    if raise_error:
        assert (
            len(column) == 1
        ), "Dataframe should contain ONLY one {} column, but found: {}".format(
            name, column
        )
    else:
        if len(column) == 0:
...
def get_column(column, data, raise_error=True, modify_inplace=False):
    ...
    return get_column_(
        data, correspondence[column], column, raise_error, modify_inplace
    )
...
def get_column_prob(data):
    ...
    if guess == None:
        logging.info(
            "No logits column found with standard names, trying to guess the probability column"
        )
        ...
        prob_columns = [
            ...
            if (
                min_max[col]["min"] >= 0
                and min_max[col]["max"] <= 1
                and min_max[col]["min"] != 0
                and min_max[col]["max"] != 1
            )
        ]
        if len(prob_columns) == 1:
            ...
            logging.info(
                "More than one column with values between 0 and 1 found, please specify the column with probabilities"
            )
            return None
        else:
            logging.info(
                "Could not guess the column with probabilities, please specify it manually"
            )
    return guess
...
def main():
    parser = argparse.ArgumentParser(
        description="Process a CSV file with MolCompass Viewer."
    )
    parser.add_argument(
        "file",
        type=str,
    ...
    parser.add_argument(
        "--demo",
        action="store_true",
        help="Run with the demo file data/endocrine/er.csv",
    )
    parser.add_argument(
        "--precompute",
        action="store_true",
        help="Whether to precompute certain data (default: False)",
    )
    parser.add_argument(
        "--log_level", type=str, default="ERROR", help="Logging level (default: ERROR)"
    )
    ...
    if args.demo:
        with as_file(
            files("molcompview").joinpath(
                join("data", "endocrine-receptor", "endocrine.csv")
            )
        ) as p:
            file_path = p
...
def main_logic(file, precompute=False, log_level="ERROR"):
    def make_dropdown(useful_columns):
        return (
            ...
            else html.Div("No columns for coloring found")
        )

    if not isfile(file):
        raise FileNotFoundError(
            "File {} not found, please specify a csv file".format(file)
        )
    ...
    if not isfile(processed_file_name):
        logging.info(
            "File {} not found in cache, processing it".format(processed_file_name)
        )
    ...
    useful_columns = [
        col
        for col in data.columns
        if col not in [__x_name__, __y_name__, __smiles_name__]
    ]
    ...
    app.layout = dbc.Container(
        [
            ...
            dbc.Row(
                [dbc.Col(molcompass_layout(useful_columns), className="g-0")],
                id="main-layout",
            ),  # Main
        ],
        fluid=True,
